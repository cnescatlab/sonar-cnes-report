...
public class FullWorkTest extends MasterTest {
    ...
    public void completeProcessTest() throws Exception {
        ...
        for(QualityProfile qp : superReport.getQualityProfiles()) {
            profileExporter.export(qp.getConf(), params, params.get(StringManager.REPORT_PATH), qp.getKey());
        }
        // export the quality gate
        gateExporter.export(superReport.getQualityGate().getConf(),params,params.get(StringManager.REPORT_PATH),superReport.getQualityGate().getName());
        String docXFilename = formatFilename("REPORT_FILENAME", superReport.getProjectName());

        // export the full docx report
        docXExporter.export(superReport, params, params.get(StringManager.REPORT_PATH), docXFilename);
        ...
        issuesExporter.export(superReport, params, params.get(StringManager.REPORT_PATH), xlsXFilename);
    }
